...
def plot_bar(cases):
    ...
    for case,text in zip(cases,texts):
        df = read_data('../../data/unit_benchmark/n_cellcount/' +case)
        Sr = get_Sr(df)
        h_l = get_h_l(df)
        theoretical_sr = theoretical(h_l)
        x = np.arange(len(Sr))  # the label locations
        rects1 = ax.bar(x + (width*i*1.2), Sr, width, alpha=0.7, label='$N_c$: ' + re.findall('\d+', case )[0] )
    ...
    fig.tight_layout()
    fig.savefig('speedup_cellcount.pdf',bbox_inches='tight',pad_inches=0.0)
    ...
    for key, item in grouped:
        ...
        for i, model in enumerate(models):
            ...
            if(i==0):
                ...
                #print("Standard max: {}".format(np.max(sliced["Mean"])))
                #print("Standard min: {}".format(np.min(sliced["Mean"])))
            else:
...
def main():
    cases = ['cellcount_40','cellcount_200','cellcount_1000','cellcount_5000','cellcount_20000','cellcount_100000']
    plot_bar(cases)
