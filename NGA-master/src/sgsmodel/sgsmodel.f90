module sgsmodel
  ...
  logical :: pope_crit
  logical :: const_Cs
  logical :: save_sfs_stats
  real(WP) :: Cs_val
  real(WP) :: schmidt_turb_fact
  ...
  real(WP), dimension(:,:,:), pointer :: Li1
  real(WP), dimension(:,:,:), pointer :: Mi1
  real(WP), dimension(:,:,:), pointer :: Li2
  real(WP), dimension(:,:,:), pointer :: Mi2
  real(WP), dimension(:,:,:), pointer :: Li3
  real(WP), dimension(:,:,:), pointer :: Mi3
  ...
  real(WP), dimension(:,:,:), pointer :: tke_sgs

  ! Subfilter stresses for statistics
  real(WP), dimension(:,:,:), pointer :: SFSxx
  real(WP), dimension(:,:,:), pointer :: SFSyy
  real(WP), dimension(:,:,:), pointer :: SFSzz
  real(WP), dimension(:,:,:), pointer :: SFSxy

  ! Subfilter scalar fluxes for statistics
  real(WP), dimension(:,:,:,:), pointer :: SFFx
  real(WP), dimension(:,:,:,:), pointer :: SFFy
...
contains  
  ...
  subroutine compute_Hij
    ...
    if (grad_model .and. .not.const_Cs) call sgs_gradient_compute_Hij
  ...
  subroutine compute_Hi(isc)
    ...
    ! If gradient model do something
    if (grad_model .and. .not.const_Cs) call sgs_gradient_compute_Hi(isc)
  ...
  subroutine sgsmodel_LM_MM_VISC
    ...
    ! All done if using constant coefficients
    if (const_Cs) return
...
!!$    call strainrate_compute(Stij1,Stij2,Stij3,Stij4,Stij5,Stij6,isc)
    ...
    do k=kmin_,kmax_
       do j=jmin_,jmax_
          do i=imin_,imax_
             ...
             Mi1(i,j,k) = 2.0_WP * delta_3D(i,j)**2 * (FrhoStGi(1) - ratio_3D(i,j)**2  * FRHO(i,j,k) * FSt(i,j,k) * FGi(1))
             Mi2(i,j,k) = 2.0_WP * delta_3D(i,j)**2 * (FrhoStGi(2) - ratio_3D(i,j)**2  * FRHO(i,j,k) * FSt(i,j,k) * FGi(2))
             Mi3(i,j,k) = 2.0_WP * delta_3D(i,j)**2 * (FrhoStGi(3) - ratio_3D(i,j)**2  * FRHO(i,j,k) * FSt(i,j,k) * FGi(3))
    ...
    do k=kmin_,kmax_
       do j=jmin_,jmax_
          do i=imin_,imax_
             ...
             Li1(i,j,k) = FrhoUtiSC(1) - FrhoSC(i,j,k) * FUi(i,j,k)
             Li2(i,j,k) = FrhoUtiSC(2) - FrhoSC(i,j,k) * FVi(i,j,k)
             Li3(i,j,k) = FrhoUtiSC(3) - FrhoSC(i,j,k) * FWi(i,j,k)

             ! Remove Hi from Li
             Li1(i,j,k) = Li1(i,j,k) - Hi1(i,j,k)
             Li2(i,j,k) = Li2(i,j,k) - Hi2(i,j,k)
             Li3(i,j,k) = Li3(i,j,k) - Hi3(i,j,k)
    ...
    do k=kmin_,kmax_
       do j=jmin_,jmax_
          do i=imin_,imax_
             ...
             LM(i,j,k) = Li1(i,j,k)*Mi1(i,j,k) + Li2(i,j,k)*Mi2(i,j,k) + Li3(i,j,k)*Mi3(i,j,k)
             
             ! Compute MiMi = sum Mi.Mi
             MM(i,j,k) = Mi1(i,j,k)*Mi1(i,j,k) + Mi2(i,j,k)*Mi2(i,j,k) + Mi3(i,j,k)*Mi3(i,j,k)
  ...
  ! -----------------------------
  subroutine sgsmodel_apply_border(A,sym)
    implicit none
    real(WP), dimension(imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_) :: A
    character(len=*), intent(in) :: sym
    
    ! Update Ghost Cells
    call boundary_update_border(A,sym,'ym')
...
subroutine sgsmodel_init
  ...
  logical :: default_pope, isdefined
  ...
  allocate(Li1(imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_))
  allocate(Li2(imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_))
  allocate(Li3(imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_))

  allocate(Mi1(imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_))
  allocate(Mi2(imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_))
  allocate(Mi3(imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_))
  
  ! Constant Smagorinsky or Dynamic Model
  call parser_read('Use constant Cs', const_Cs,.false.)
  if (const_Cs) then
     ! Constant coefficient
     call monitor_log("Using constant Smagorinsky coefficient")
     call parser_read('Smagorinsky coefficient',Cs_val)
     call parser_read('Turbulent Schmidt number',schmidt_turb_fact)
     call parser_is_defined('SGS averaging',isdefined)
     if (isdefined) then
        call die('sgsmodel_init: cannot use constant coefficient and SGS averaging')
     end if
     LM         = Cs_val
     MM         = 1.0_WP
     schmidt_turb_fact = 1.0_WP / schmidt_turb_fact

     ! Do we use a "Clark" term?
     call parser_read('Use gradient model',grad_model)
     if (grad_model) call sgs_gradient_init
     
  else 
     ! Dynamic Model: there is more to do
     call parser_is_defined('Smagorinsky coefficient',isdefined)
     if (isdefined) then
        call die('sgsmodel_init: cannot use dynamic model and specify Smagorinsky coefficient')
     end if
     schmidt_turb_fact = 1.0_WP
  ...
  ! Decide whether to save subfilter stresses for statistics
  call parser_read('Save SFS stress statistics',save_sfs_stats, .false.)

  if (save_sfs_stats) then
     ! Allocate arrays for subfilter stresses for statistics
     allocate(SFSxx(imin_:imax_,jmin_:jmax_,kmin_:kmax_))
     allocate(SFSyy(imin_:imax_,jmin_:jmax_,kmin_:kmax_))
     allocate(SFSzz(imin_:imax_,jmin_:jmax_,kmin_:kmax_))
     allocate(SFSxy(imin_:imax_,jmin_:jmax_,kmin_:kmax_))

     ! Allocate arrays for subfilter stresses for statistics
     if (nscalar.ge.1) then
        allocate(SFFx (imin_:imax_,jmin_:jmax_,kmin_:kmax_,nscalar))
        allocate(SFFy (imin_:imax_,jmin_:jmax_,kmin_:kmax_,nscalar))
     end if
  end if
...
subroutine sgsmodel_eddyVISC
  ...
  ! Calculate dynamic Smagorinsky coefficient
  if (.not.const_Cs) then
...
!!$     do j=jmino_,jmaxo_
!!$        do i=imino_,imaxo_
!!$           tke_sgs(i,j,k) = S(i,j,k) * Cs_visc(i,j,k) * delta_3D(i,j)
!!$        end do
!!$     end do
!!$  end do
!!$  !$OMP END PARALLEL DO
  ...
  if (use_sgs) then
     ...
     do k=kmino_,kmaxo_
        do j=jmino_,jmaxo_
           do i=imino_,imaxo_
              !VISC(i,j,k) = max(VISC(i,j,k),0.0_WP)
              VISC(i,j,k) = max(VISC(i,j,k),-VISCmol(i,j,k))
...
subroutine sgsmodel_eddyDIFF
  ...
  ! NOTE: diffusivity should be calculated based on scalars at mipoint of scalar steps
  ! The computation of midpoint scalars is now in scalar_step
  !
  ...
  do isc=1,nscalar
     ...
     do k=kmino_,kmaxo_
        do j=jmino_,jmaxo_
           do i=imino_,imaxo_
              if (MM(i,j,k).ne.0.0_WP) then
                 Cs_diff(i,j,k) = LM(i,j,k)/MM(i,j,k) * schmidt_turb_fact
              else
...
!!$              DIFF(i,j,k,isc) = RHO(i,j,k) * St(i,j,k) * Cs_diff(i,j,k) * delta_3D(i,j)**2
  ...
  do isc=1,nscalar
     ...
     do k=kmino_,kmaxo_
        do j=jmino_,jmaxo_
           do i=imino_,imaxo_
              !DIFF(i,j,k,isc) = max(DIFF(i,j,k,isc),0.0_WP)
              DIFF(i,j,k,isc) = max(DIFF(i,j,k,isc),-DIFFmol(i,j,k,isc))
  ...
  ! This is now done after the function call in scalar step
...
! ============================================== !
! Compute stresses and fluxes for postprocessing !
! ============================================== !
subroutine sgsmodel_poststep
  use sgsmodel
  use interpolate
  use metric_generic
  implicit none
  ...
  !$OMP PARALLEL DO PRIVATE(divg_local)
  do k=kmin_,kmax_
     do j=jmin_,jmax_
        do i=imin_,imax_
           divg_local = sum(grad_xm(i,j,:)*Ui(i-stp:i+stp,j,k)) + sum(grad_ym(i,j,:)*Vi(i,j-stp:j+stp,k)) + sum(grad_zm(i,j,:)*Wi(i,j,k-stp:k+stp))
           SFSxx(i,j,k) = -2.0_WP*(VISC(i,j,k)-VISCmol(i,j,k))*(sum(grad_xm(i,j,:)*Ui(i-stp:i+stp,j,k))-1.0_WP/3.0_WP*divg_local)
           SFSyy(i,j,k) = -2.0_WP*(VISC(i,j,k)-VISCmol(i,j,k))*(sum(grad_ym(i,j,:)*Vi(i,j-stp:j+stp,k))-1.0_WP/3.0_WP*divg_local)
           SFSzz(i,j,k) = -2.0_WP*(VISC(i,j,k)-VISCmol(i,j,k))*(sum(grad_zm(i,j,:)*Wi(i,j,k-stp:k+stp))-1.0_WP/3.0_WP*divg_local)
           SFSxy(i,j,k) = -(VISC(i,j,k)-VISCmol(i,j,k))*(sum(grad_ym(i,j,:)*Ui(i,j-stp:j+stp,k))+sum(grad_xm(i,j,:)*Vi(i-stp:i+stp,j,k)))
        end do
     end do
  end do
  !$OMP END PARALLEL DO
  ...
  if (nscalar.ge.1) then
     do isc=1,nscalar
        ...
        do k=kmin_,kmax_
           do j=jmin_,jmax_
              do i=imin_,imax_
                 SFFx(i,j,k,isc) = -(DIFF(i,j,k,isc)-DIFFmol(i,j,k,isc))*sum(grad_xm(i,j,:)*SC(i-stp:i+stp,j,k,isc))
                 SFFy(i,j,k,isc) = -(DIFF(i,j,k,isc)-DIFFmol(i,j,k,isc))*sum(grad_ym(i,j,:)*SC(i,j-stp:j+stp,k,isc))
              end do
           end do
        end do
        !$OMP END PARALLEL DO
  ...
  ! Gradient model
  if (grad_model) call sgs_gradient_poststep
  
  ! Stop the timer
  call timing_stop('sgsmodel')
  
  return
end subroutine sgsmodel_poststep
