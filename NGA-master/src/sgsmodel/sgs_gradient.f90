...
contains
  ...
  subroutine compute_gradient_velocity
    ...
    do k=kmin_,kmax_
       do j=jmin_,jmax_
          do i=imin_,imax_
             ...
             ! Subtract out the trace
             trace = Hij1(i,j,k) + Hij2(i,j,k) + Hij3(i,j,k)
             Hij1(i,j,k) = Hij1(i,j,k) - 1.0_WP/3.0_WP*trace
             Hij2(i,j,k) = Hij2(i,j,k) - 1.0_WP/3.0_WP*trace
             Hij3(i,j,k) = Hij3(i,j,k) - 1.0_WP/3.0_WP*trace
    ...
    ! Update Ghost Cells
    call sgsmodel_apply_border(Hij1,'+')
    call sgsmodel_apply_border(Hij2,'+')
    call sgsmodel_apply_border(Hij3,'+')
    call sgsmodel_apply_border(Hij4,'-')
    call sgsmodel_apply_border(Hij5,'+')
    call sgsmodel_apply_border(Hij6,'-')
  ...
  subroutine compute_gradient_scalar(isc)
    ...
    do k=kmin_,kmax_
       do j=jmin_,jmax_
          do i=imin_,imax_
             ...
             dUdx(:,2) = dUdx(:,2)*dy(j)
             dUdx(:,3) = dUdx(:,3)*dz_c(j)
             dZdx(1) = sum(grad_xm(i,j,:)*SC(i-stp:i+stp,j,k,isc))*dx(i)
             dZdx(2) = sum(grad_ym(i,j,:)*SC(i,j-stp:j+stp,k,isc))*dy(j)
             dZdx(3) = sum(grad_zm(i,j,:)*SC(i,j,k-stp:k+stp,isc))*dz_c(j)

             Hi1(i,j,k) = -RHO(i,j,k)*sum(dZdx(:)*dUdx(1,:))/12.0_WP
             Hi2(i,j,k) = -RHO(i,j,k)*sum(dZdx(:)*dUdx(2,:))/12.0_WP
             Hi3(i,j,k) = -RHO(i,j,k)*sum(dZdx(:)*dUdx(3,:))/12.0_WP
    ...
    ! Update Ghost Cells
    call sgsmodel_apply_border(Hi1,'+')
    call sgsmodel_apply_border(Hi2,'-')
    call sgsmodel_apply_border(Hi3,'-')
...
subroutine sgs_gradient_compute_Hij
  ...
  ! Update Ghost Cells
  call sgsmodel_apply_border(Gij11,'+')
  call sgsmodel_apply_border(Gij12,'-')
  call sgsmodel_apply_border(Gij13,'-')
  call sgsmodel_apply_border(Gij21,'-')
  call sgsmodel_apply_border(Gij22,'+')
  call sgsmodel_apply_border(Gij23,'+')
  call sgsmodel_apply_border(Gij31,'-')
  call sgsmodel_apply_border(Gij32,'+')
  call sgsmodel_apply_border(Gij33,'+')
...
subroutine sgs_gradient_compute_Hi(isc)
  ...
  call sgsmodel_apply_border(GSi1,'+')
  call sgsmodel_apply_border(GSi2,'-')
  call sgsmodel_apply_border(GSi3,'-')
...
! ============================================== !
subroutine sgs_gradient_poststep
  use sgsmodel
  use sgs_gradient
  implicit none
  ...
  do k=kmin_,kmax_
     do j=jmin_,jmax_
        do i=imin_,imax_
           SFSxx(i,j,k) = SFSxx(i,j,k) - Hij1(i,j,k)
           SFSyy(i,j,k) = SFSyy(i,j,k) - Hij2(i,j,k)
           SFSzz(i,j,k) = SFSzz(i,j,k) - Hij3(i,j,k)
           SFSxy(i,j,k) = SFSxy(i,j,k) - Hij4(i,j,k)
        end do
     end do
  end do
  !$OMP END PARALLEL DO
  ...
  do isc=1,nscalar
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
              SFFx(i,j,k,isc) = SFFx(i,j,k,isc) - Hi1(i,j,k)
              SFFy(i,j,k,isc) = SFFy(i,j,k,isc) - Hi2(i,j,k)
           end do
        end do
     end do
     !$OMP END PARALLEL DO
  ...
  return
end subroutine sgs_gradient_poststep
