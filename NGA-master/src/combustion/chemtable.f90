...
subroutine chemtable_init(filename)
  ...
  case ('PFPVA-3D')
     if ((isc_PROG.eq.0) .or. (isc_ZMIX.eq.0)) call die('chemtable_init: PFPV-3D model requires scalars PROG and ZMIX')
  case ('UFPVA')
  ...
  case ('RPFPVAR')
     if ((isc_PROG.eq.0) .or. (isc_ZMIX.eq.0) .or. (isc_ENTH.eq.0)) call die('chemtable_init: Radiation FPV model requires scalars ZMIX, PROG, and H')
  case ('MMFPVA-Z*')
     if ((isc_ZMIX.eq.0) .or. (isc_PROG.eq.0) .or. (isc_ZSTAR.eq.0 .and. isc_FMIX.eq.0)) call die('chemtable_init: Multiple Mixture Fraction FPV model (triangular) requires scalars ZMIX, PROG, and ZSTAR')
  end select
...
subroutine chemtable_lookup(tag,Ain)
  ...
  case (2)
     ...
     case ('PFPVA')
        if (isc_ZSTAR.eq.0) then
           call chemtable_2D_lookup(tag,Ain,SC(:,:,:,isc_PROG),SC(:,:,:,isc_ZMIX) ,nxo_*nyo_*nzo_)
        else
           call chemtable_2D_lookup(tag,Ain,SC(:,:,:,isc_PROG),SC(:,:,:,isc_ZSTAR),nxo_*nyo_*nzo_)
        end if
     case default
        call die('chemtable_lookup: Wrong number of chemtable dimensions')
     end select
  case (3)
     select case(trim(combModel))
     case ('Steady Flamelet')
     ...
     case ('PFPVA-3D')
        if (isc_ZSTAR.eq.0) then
           call chemtable_3D_lookup(tag,Ain,SC(:,:,:,isc_PROG),PROGVAR           ,SC(:,:,:,isc_ZMIX)   ,nxo_*nyo_*nzo_)
        else
           call chemtable_3D_lookup(tag,Ain,SC(:,:,:,isc_PROG),PROGVAR           ,SC(:,:,:,isc_ZSTAR)  ,nxo_*nyo_*nzo_)
        end if
     case default
        call die('chemtable_lookup: Wrong number of chemtable dimensions')
     end select
  case (4)
     ...
     case ('RPFPVAR')
        call chemtable_4D_lookup(tag,Ain,SC(:,:,:,isc_PROG),PROGVAR,SC(:,:,:,isc_ZMIX), SC(:,:,:,isc_ENTH),nxo_*nyo_*nzo_)
     case ('MMFPVA-Z*')
        call chemtable_4D_lookup(tag,Ain,SC(:,:,:,isc_ZMIX),ZVAR,FMIX               ,SC(:,:,:,isc_PROG),nxo_*nyo_*nzo_)
     case default
        call die('chemtable_lookup: Wrong number of chemtable dimensions')
     end select
  end select
...
function chemtable_lookup_rho_local(i,j,k,Z1)
  ...
  case (3)
     ...
     case ('PFPVA-3D')
        call die("chemtable_lookup_rho_local: This function is not supported for PFPVA-3D model")
     end select
  case (4)
     select case(trim(combModel))
     case ('UFPVA')
     ...
     case ('RPFPVAR')
        chemtable_lookup_rho_local = chemtable_4D_lookup_rho_local(Z1,PROGVAR(i,j,k),SC(i,j,k,isc_ZMIX),SC(i,j,k,isc_ENTH))
     case ('MMFPVA-Z*')
        call die("chemtable_lookup_rho_local: This function is not supported for MMFPVA-Z* model")
        !if ( SC(i,j,k,isc_ZSTAR) .ge. Z1) then
        !   chemtable_lookup_rho_local = chemtable_4D_lookup_rho_local(SC(i,j,k,isc_ZSTAR),ZVAR(i,j,k),1.0_WP,SC(i,j,k,isc_PROG))
        !else
        !   chemtable_lookup_rho_local = chemtable_4D_lookup_rho_local(Z1,ZVAR(i,j,k), SC(i,j,k,isc_ZSTAR)/Z1 ,SC(i,j,k,isc_PROG))
        !end if
     end select
  end select
...
subroutine chemtable_source(srcSC)
  ...
  if (isc_PROGVAR.ne.0) then
     call chemtable_lookup('SRC_PRO2',tmp2)
     !$OMP PARALLEL DO
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
              srcSC(i,j,k,isc_PROGVAR) = srcSC(i,j,k,isc_PROGVAR) + dt_*2.0_WP*RHO(i,j,k)*(tmp2(i,j,k)-tmp1(i,j,k)*SC(i,j,k,isc_PROG))
           end do
        end do
     end do
     !$OMP END PARALLEL DO
  end if
  if (isc_PROG2.ne.0) then
     call chemtable_lookup('SRC_PRO2',tmp1)
     !$OMP PARALLEL DO
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
              srcSC(i,j,k,isc_PROG2) = srcSC(i,j,k,isc_PROG2) + dt_*2.0_WP*RHO(i,j,k)*tmp1(i,j,k)
           end do
        end do
     end do
     !$OMP END PARALLEL DO
  ...
  if (isc_ENTH.ne.0) then
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
              srcSC(i,j,k,isc_ENTH) = srcSC(i,j,k,isc_ENTH) + dt_*RHO(i,j,k)*min(tmp1(i,j,k), 0.0_WP)
           end do
        end do
     end do
     !$OMP END PARALLEL DO
