...
contains
  ...
  subroutine metric_generic_allocate
    ...
    ! Inside only
    allocate(interp_u_xm(imin_-st1:imax_+st2,jmin_-st1-1:jmax_+st2,-st1:st2))
    allocate(interp_v_ym(imin_-st1:imax_+st2,jmin_-st1-1:jmax_+st2,-st1:st2))
    allocate(interp_w_zm(imin_-st1:imax_+st2,jmin_-st1-1:jmax_+st2,-st1:st2))
...
!!$          ! JFM 9/22/16
!!$          ! This needs to be better -- only works for 2nd order stencils
!!$          interp_uvw_x(i,j,-st2) = 1.0_WP-0.5_WP*dx(i-st2)/dxm(i-st2)
!!$          interp_uvw_x(i,j, st1) =        0.5_WP*dx(i-st2)/dxm(i-st2)
!!$          interp_uvw_y(i,j,-st2) = 1.0_WP-0.5_WP*dy(j-st2)/dym(j-st2)
!!$          interp_uvw_y(i,j, st1) =        0.5_WP*dy(j-st2)/dym(j-st2)
    ...
    do j=jmin_-st1,jmax_+st2
       do i=imin_-st1,imax_+st2+1
!!$          call hofdd(2,xm(i-st2:i+st1),x(i),grad_x(i,j,:))
!!$          call hofdd(2,ym(j-st2:j+st1),y(j),grad_y(i,j,:))
!!$          call hofdd(2,zm(kmin_-st2:kmin_+st1),z(kmin_),grad_z(i,j,:))

!!$
!!$          ! JFM 9/22/16
!!$          ! This needs to be better -- only works for 2nd order stencils
!!$          w2 = 0.5_WP*dx(i-1)/dxm(i-1); w1 = 1.0_WP-w2
!!$          d1 = 0.5_WP*dx(i-1); d2 = 0.5_WP*dx(i)
!!$          grad_x(i,j,-st2) = 0.5_WP*((w1-1.0_WP)/d1 - w1/d2)
!!$          grad_x(i,j, st1) = 0.5_WP*(w2/d1 + (1.0_WP-w2)/d2)
!!$
!!$          w2 = 0.5_WP*dy(j-1)/dym(j-1); w1 = 1.0_WP-w2
!!$          d1 = 0.5_WP*dy(j-1); d2 = 0.5_WP*dy(j)
!!$          grad_y(i,j,-st2) = 0.5_WP*((w1-1.0_WP)/d1 - w1/d2)
!!$          grad_y(i,j, st1) = 0.5_WP*(w2/d1 + (1.0_WP-w2)/d2)
!!$ 
