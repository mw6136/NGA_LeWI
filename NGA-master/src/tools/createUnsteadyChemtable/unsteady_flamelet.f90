...
! ============================================== !
subroutine unsteady_flamelet_init
  ...
  case ('RFPVA')
     ...
     if (vartran) then
        nvar_in = nvar_in + 6
        input_name(nvar_in-5) = 'Z2RHODOT'
        input_name(nvar_in-4) = 'ZSRC_ZMIX'
     else
        nvar_in = nvar_in + 4
     end if
     input_name(nvar_in-3) = 'SRC_PROG'
     input_name(nvar_in-2) = 'PROG'
  ...
  return
end subroutine unsteady_flamelet_init
...
! ================================================ !
subroutine unsteady_flamelet_readfile(filename)
  use unsteady_flamelet
  use fileio
  implicit none
  ...
  do while(index(buffer,'body').eq.0)
     ...
     if (index(buffer,'gridPoints').ne.0) then
        ...
        if (trim(combModel).eq.'RFPVA') then
           if (vartran) then
              Z2RHODOT  => input_data(:,nvar_in-5)
              ZSRC_ZMIX => input_data(:,nvar_in-4)
           end if
           SRC_PROG => input_data(:,nvar_in-3)
           PROG     => input_data(:,nvar_in-2)
  ...
  loop1:do while (ierr .eq. 0)
     ...
     ! Is it the enthalpy?
     if (trim(combModel).eq.'RFPVA') then
!!$        if (trim(varname).eq.'TotalEnthalpy' .and. found(nvar_in).eq..false.) then
!!$           read(line,*) tmp
!!$           ENTHALPY = tmp
!!$           found(nvar_in) = .true.
!!$        end if
...
!!$           end if
  ...
  do var=1,nvar_in
     if (trim(input_name(var)).eq.'ProgDer') then
        found(var) = .true.
        input_data(1,var) = (PROG(2)-PROG(1)) / (Z(2)-Z(1))
  ...
  if (vartran .and. found(index_enthdot)) then
  ...
  if (trim(combModel).eq.'RFPVA' .and. sootrad) then
  ...
  return
end subroutine unsteady_flamelet_readfile


! ========================================= !
! Deallocate the data array for new reading !
! ========================================= !
subroutine unsteady_flamelet_cleanup
  use unsteady_flamelet
  implicit none
  ...
  return
end subroutine unsteady_flamelet_cleanup
