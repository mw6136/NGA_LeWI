...
!  close(iclose(iunit1))
  ...
  !w_snap = 344 !w_snap +320
  !w_var = 1
  w_var = CEILING(REAL(irank,WP)/REAL(40,WP))
  print*, irank, w_snap
  ...
  names(2) = 'vort_y_'
  names(3) = 'vort_z_'
  fname = trim(folder)//trim(adjustl(names(w_var)))
  ...
  do l = 1,n_snap
     ...
     i = start_file +(l-1)
     filename1 = trim(folder)//trim(adjustl(names(w_var)))
