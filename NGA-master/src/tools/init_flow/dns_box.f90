...
subroutine dns_box_data
  ...
  print *, 'Done velocity spectrum init.'
  
  ! Compute spectrum
  nk = nx/2+1
  allocate(ak(nk,ny,nz),bk(nk,ny,nz))
  !$OMP PARALLEL DO PRIVATE(j,i,rand,psr,ps1,ps2,kx,ky,kz,kk,energy_spec)
  do k=1,nz
     do j=1,ny
        do i=1,nk
           ! Random numbers
  ...
  do dim=1,3
     ...
     print *, 'Fourier coefficients i=',dim

     ! Compute the Fourier coefficients
     !$OMP PARALLEL DO PRIVATE(j,i,kx,ky,kz,kk,kk2)
     do k=1,nz
        do j=1,ny
           do i=1,nk
  ...
  print *, 'Done Fourier coefficients'
  
  ! Inverse Fourier transform
  allocate(Cbuf(nk,ny,nz))
  print *, 'Allocated Cbuf'
  allocate(Rbuf(nx,ny,nz))
  print *, 'Allocated Rbuf'
...
subroutine dns_box_check(spectrum,Ut,ke)
  ...
  allocate(spect(2,nx+1))
  spect = 0.0_WP
  call get_spectrum(U,spect)
  call get_spectrum(V,spect)
  call get_spectrum(W,spect)
