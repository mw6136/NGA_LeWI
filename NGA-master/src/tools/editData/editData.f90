program editData
  ...
  integer :: nx,ny,nz,nz2,nvar,nvar2,n_del
  ...
  integer :: iunit1,iunit2,ierr,var,choice,iunit3,var2,nrep,irep
  ...
  print*, "8. Repeat data in z"
  print*, "9. Scale channel data file"
  print "(a9,$)", "Choice : "
  read "(i1)", choice
  ...
  case (8)  ! Repeat data in z
     print "(a24,$)", "Number of repetitions : "
     read "(i1)", nrep
     print *, "Don't forget to make the matching config file!"
     nz2 = nz*nrep
     print*,'New grid :',nx,'x',ny,'x',nz2
     call BINARY_FILE_OPEN(iunit2,trim(filename2),"w",ierr)
     call BINARY_FILE_WRITE(iunit2,nx,1,kind(nx),ierr)
     call BINARY_FILE_WRITE(iunit2,ny,1,kind(ny),ierr)
