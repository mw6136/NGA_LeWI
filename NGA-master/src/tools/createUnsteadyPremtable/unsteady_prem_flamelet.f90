module unsteady_prem_flamelet
  ...
  ! Product conversion table logical
  logical :: prodconv

  ! Adiabaticity logical
  logical :: adflt
...
subroutine unsteady_prem_flamelet_init
  ...
  case ('RPFPVA')
     ...
     call parser_read('Product conversion table',prodconv,.false.)
...
subroutine unsteady_prem_flamelet_readfile(filename)
  ...
  character(len=16*str_long) :: line
  ...
  do while(index(buffer,'body').eq.0)
     ...
     ! Determine adiabatic flamelets
     if (trim(combModel).eq.'RPFPVA') then
        if (prodconv) then
           if (index(buffer,'adiabatic').ne.0) then
              adflt = .true.
           endif
        endif
     endif
...
!!$           found(nvar_in) = .true.
!!$        end if
        ! If we're using product conversion, prepare to assign H later
        if (prodconv) then
           if (trim(varname).eq.'TotalEnthalpy') then
              if (adflt) then
                 read(line,*) tmp
