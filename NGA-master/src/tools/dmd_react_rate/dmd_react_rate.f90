...
program dmd_vort
  ...
  integer :: n_snap, start_file, step_file, name_snap, nvars
...
!  close(iclose(iunit1))
  ...
  w_snap = MOD(irank,80)
  if (w_snap == 0) w_snap = 80
  w_snap = w_snap +320
  w_var = 1
  print*, irank, var_list

  fname = trim(folder)//"rr_"
  fname = trim(fname)//trim(adjustl(names(var_list(w_var))))
  ...
  do l = 1,n_snap
     i = start_file +(l-1)
     filename1 = trim(folder)//trim(adjustl(names(var_list(w_var))))//"_"
