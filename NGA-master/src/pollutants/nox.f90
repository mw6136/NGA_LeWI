  ...
  do k=kmin_,kmax_
     do j=jmin_,jmax_
        do i=imin_,imax_
           ...
           srcSC_(i,j,k,isc_NOX) = srcSC_(i,j,k,isc_NOX) + dt_ * tmp2(i,j,k)
           ! Rescale consumption with transported value
           srcSC_(i,j,k,isc_NOX) = srcSC_(i,j,k,isc_NOX) + dt_ * tmp3(i,j,k) * (SC_(i,j,k,isc_NOX)/max(tmp1(i,j,k),1e-60))
           ! Rescale consumption with transported value (old way, BAD!)
           !srcSC_(i,j,k,isc_NOX) = tmp2(i,j,k) - tmp3(i,j,k) * (SC_(i,j,k,isc_NOX)/max(tmp1(i,j,k),1e-60))
        end do
     end do
  end do
  !$OMP END PARALLEL DO
