...
contains
  ...
  function soot_fracmom(i,j)
    ...
    elseif (nMoments.eq.7) then
       ...
       if (m00.lt.1e-25_WP .or. m10.lt.1e-25_WP .or. m01.lt.1e-25_WP .or. &
            m20.lt.1e-25_WP .or. m11.lt.1e-25_WP .or. m02.lt.1e-25_WP) then
          f1 = mom(1)
  ...
  function soot_beta_cond()
    ...
    if (use_soot_sgs) then
       ! If all moments are 0.0, set omega = 1.0 (no soot)
       if (all(mom .eq. 0.0_WP)) then
          omega = 1.0_WP
       else
          omega = min( max(1.0_WP - (mom(1)**2/mom(nMoments+1)), 0.0_WP), 1.0_WP - 1.0e-8_WP)
       end if
    end if
    
    ! Only compute soot_beta_cond if omega .ne. 1.0
    if (omega .eq. 1.0_WP) then
       soot_beta_cond = 0.0_WP
       mom = 0.0_WP
    else
  ...
  subroutine soot_pah_molecules
    ...
    delta = betaC**2 + 4.0_WP*betaN*max(prodRate,0.0_WP)
  ...
  subroutine soot_hmom_source_solid
    ...
    do i=1,nMoments-1
       ...
       ! Only compute following source terms if omega .ne. 1.0
       if (omega .ne. 1.0_WP) then
          ! Coagulation
  ...
  subroutine soot_hmom_source_gas(SC_,srcSC_,srcP_)
    ...
    select case(trim(chemistry))
       case ('chemtable')
          ...
          if (isc_ZSTAR.ne.0) srcSC_(isc_ZSTAR) = srcSC_(isc_ZSTAR) + dt_*max(SC_(isc_ZSTAR),1.0e-8_WP)/max(SC_(isc_ZMIX),1.0e-8_WP)*zsrc*dens
          ...
          if (use_PAH) then
             ...
             srcSC_(isc_PAH) = srcSC_(isc_PAH) + dt_*min( (min(max(SC_(isc_PAH),1.0e-60_WP),1.0_WP)/ypah)**2, 1.0e2_WP )
          end if
       case ('finite chem')
          ! PAH equations
          ...
          srcSC_(isc_T) = srcSC_(isc_T) - dt_*SC_(isc_T)*rhodot
    end select
...
subroutine soot_hmom_init
  ...
  ! Need viscosity * sqrt molar mass / (density * sqrt temperature) for slip factor constant
  av = 1.0_WP - (2.0_WP / Df)
  as = (3.0_WP / Df) - 1.0_WP
  FitC = 2.0_WP/3.0_WP
  FitExp = -0.2043_WP
  ...
  ! Define the moments used for HMOM
      ...
      case ('chemtable')
         ...
         if (use_pah) then
            ...
            do k=kmin_,kmax_
               do j=jmin_,jmax_
                  do i=imin_,imax_
                     tmp7(i,j,k) = max(tmp7(i,j,k), 0.0_WP) * min( ( min(  max(SC_(i,j,k,isc_PAH),1.0e-60_WP),1.0_WP )/max(tmp8(i,j,k), 1.0e-60_WP))**2, 1.0e2_WP )
                  end do
               end do
            end do
            !$OMP END PARALLEL DO
         ...
         call chemtable_lookup('SRC_ZMIX',tmp10)
           ...
           if (mask(i,j).ne.1) then
              ...
              select case(trim(chemistry))
                 case ('chemtable')
                    ...
                    wCoeff = max(tmp4(i,j,k), 0.0_WP)
                    oxCoeff = max(tmp5(i,j,k), 0.0_WP)
                    o2Coeff = max(tmp6(i,j,k), 0.0_WP)
                    ProdRate = tmp7(i,j,k)
                 case ('finite chem')
                    sqrtT = sqrt(T(i,j,k))
...
subroutine soot_hmom_poststep
  ...
  if (use_pah) then
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
              tmp7(i,j,k) = max(tmp7(i,j,k), 0.0_WP) * min(  ( min(max(SC(i,j,k,isc_PAH),1.0e-60_WP),1.0_WP)/max(tmp8(i,j,k), 1.0e-60_WP) )**2, 1.0e2_WP  )
           end do
        end do
     end do
     !$OMP END PARALLEL DO
     ...
     do j=jmino_,jmaxo_
        do i=imino_,imaxo_
           ...
           if (mask(i,j).eq.1) then
              ...
              ! Only compute following terms if omega .ne. 1.0
              if (omega .eq. 1.0_WP) then
                 mom = 0.0_WP

                 ! Number density
                 numdens(i,j,k) = 0.0_WP

                 ! Volume fraction
                 volfrac(i,j,k) = 0.0_WP

                 ! Primary particle diameter
                 partdiam(i,j,k) = 0.0_WP

                 ! Number of primary particles per aggregate
                 partaggr(i,j,k) = 0.0_WP
              else
                 mom = mom / (1.0_WP - omega)
              ...
              ! Intermittency
              ! Include no soot case (omega = 1.0)
              if (omega .eq. 1.0_WP) then
                 intermit(i,j,k) = 1.0_WP
              else
