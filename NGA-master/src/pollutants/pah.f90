  ...
  do k=kmin_,kmax_
     do j=jmin_,jmax_
        do i=imin_,imax_
           ...
           srcSC_(i,j,k,isc_PAH) = srcSC_(i,j,k,isc_PAH) + dt_ * min(tmp3(i,j,k),0.0_WP) * &
                min(  min( max(SC_(i,j,k,isc_PAH),1.0e-60_WP),1.0_WP )/max(tmp1(i,j,k),1.0e-60_WP),1.0e1_WP  )
        end do
     end do
  end do
  !$OMP END PARALLEL DO
...
subroutine pah_poststep
  ...
  do k=kmino_,kmaxo_
     do j=jmino_,jmaxo_
        do i=imino_,imaxo_
           SC(i,j,k,isc_PAH) = min( max(SC(i,j,k,isc_PAH),1.0e-60_WP), 1.0_WP )
        end do
     end do
  end do
  !$OMP END PARALLEL DO
