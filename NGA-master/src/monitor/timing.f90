...
!!$     ! JFM 11/29 catch pressure solver if it takes too long
!!$     if ((trim(timers(itimer)%name).eq.'pressure').and.(mval(2*itimer+2).gt.80.0_WP)) then
!!$        call die('timing_monitor: pressure solver took too long')
!!$     end if
  end do
