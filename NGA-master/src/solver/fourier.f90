...
subroutine fourier_rhs
  ...
  if (fft_x) then
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           in_x = RP(:,j,k)
           call dfftw_execute_r2r(fplan_x,in_x,out_x)
           RP(:,j,k) = out_x
        end do
     end do
     !$OMP END DO
  ...
  if (fft_y) then
     ...
     do k=kmin_,kmax_
        do i=imin_,imax_
           in_y = RP(i,:,k)
           call dfftw_execute_r2r(fplan_y,in_y,out_y)
           RP(i,:,k) = out_y
        end do
     end do
     !$OMP END DO
  ...
  if (fft_z) then
     ...
     do j=jmin_,jmax_
        do i=imin_,imax_
           in_z = RP(i,j,:)
           call dfftw_execute_r2r(fplan_z,in_z,out_z)
           RP(i,j,:) = out_z
        end do
     end do
     !$OMP END DO
  ...
  if (fft_x) then
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           ...
           call dfftw_execute_r2r(bplan_x,in_x,out_x)
  ...
  if (fft_y) then
     ...
     do k=kmin_,kmax_
        do i=imin_,imax_
           ...
           call dfftw_execute_r2r(bplan_y,in_y,out_y)
  ...
  if (fft_z) then
     ...
     do j=jmin_,jmax_
        do i=imin_,imax_
           ...
           call dfftw_execute_r2r(bplan_z,in_z,out_z)
