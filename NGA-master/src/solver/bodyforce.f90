...
contains
  ...
  subroutine bodyforce_hit
    ...
    call parser_read("Use Initial TKE",useTKE0,.true.)
    ...
    ! Following Carrol and Blanquart
    if (useTKE0) then
       tke_0 = tke_0/tke
       do k=kmin_,kmax_
          do j=jmin_,jmax_
             do i=imin_,imax_
                srcU(i,j,k) = dt_uvw*Ahit*rhoU(i,j,k)*(tke_0)
                srcV(i,j,k) = dt_uvw*Ahit*rhoV(i,j,k)*(tke_0)
                srcW(i,j,k) = dt_uvw*Ahit*rhoW(i,j,k)*(tke_0)
             end do
          end do
       end do
       tke_0 = tke_0*tke
    else ! Following Meneveau PoF 17 (2005)
       do k=kmin_,kmax_
          do j=jmin_,jmax_
             do i=imin_,imax_
                srcU(i,j,k) = dt_uvw*Ahit*rhoU(i,j,k)
                srcV(i,j,k) = dt_uvw*Ahit*rhoV(i,j,k)
                srcW(i,j,k) = dt_uvw*Ahit*rhoW(i,j,k)
             end do
          end do
       end do
    end if
