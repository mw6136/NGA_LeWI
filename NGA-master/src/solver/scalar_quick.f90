...
subroutine scalar_quick_residual
  ...
  ! JFM added 9/28/16 for MMS
  real(WP), parameter :: pi = 4.0_WP*atan(1.0_WP)

  if (AF_jac) then
     rhs_fc = 0.0_WP
     ! Get rhs from finite chem
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
              ...
              if (mms) then
                 rhs = rhs &
                      + pi*RHO(i,j,k)/4.0e-5_WP*sin(2.0_WP*pi*xm(i)/0.025_WP)*cos(2.0_WP*pi*time/4.0e-5_WP) &
                      + pi*RHO(i,j,k)*U(i,j,k)/0.025_WP *cos(2.0_WP*pi*xm(i)/0.025_WP)*sin(2.0_WP*pi*time/4.0e-5_WP) &
                      + 2.0_WP*pi**2*DIFF(i,j,k,isc)/0.025_WP**2*sin(2.0_WP*pi*xm(i)/0.025_WP)*sin(2.0_WP*pi*time/4.0e-5_WP) 
              end if

              if (AF_jac) then
                 rhs = rhs + rhs_fc(i,j,k,isc)
              end if
...
subroutine scalar_quick_inverse
  ...
  if (AF_jac) then
     ! Use approximately factorized Jacobian
     call finitechem_solve_AF
  end if
