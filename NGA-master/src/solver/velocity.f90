...
!!$  W = alpha*W+(1.0_WP-alpha)*Wold
  ...
  ! Raise an error if Infinity detected
  if ( (abs(max_resU) .eq. 1.0_WP/0.0_WP) .or. &
       (abs(max_resV) .eq. 1.0_WP/0.0_WP) .or. &
       (abs(max_resW) .eq. 1.0_WP/0.0_WP) ) then
     call die("velocity_step: dying because an Infinity was detected - would lead to HYPRE error")
  end if
  ...
  do k=kmin_,kmax_
     do j=jmin_,jmax_
        do i=imin_,imax_
           ...
           if(xper.eq.0.and.i.eq.imin) then
              ResU(imin,j,k) = 0.0_WP
           endif
        end do
     end do
  end do
