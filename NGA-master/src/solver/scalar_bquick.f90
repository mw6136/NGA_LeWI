module scalar_bquick
  ...
  ! Temporary array
  real(WP), dimension(:), pointer :: SC_bquick

  !$OMP THREADPRIVATE(SC_bquick)
...
subroutine scalar_bquick_check_bounds
  ...
  do isc=1,nscalar
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
              SC_bquick = ResSC(i,j,k,:)
              call combustion_check_bounds(SC_bquick,i,j,k,isc,tmpi)
              !call spray_check_bounds(SC_bquick,i,j,k,isc,tmpi)
              !call pollutants_check_bounds(SC_bquick,i,j,k,isc,tmpi)
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           do i=imin_,imax_
              ...
              ResSC(i,j,k,isc) = 2.0_WP*SC(i,j,k,isc)-SC_(i,j,k,isc) + ResSC(i,j,k,isc)
