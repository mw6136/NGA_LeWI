module parallel
  ...
  interface parallel_sum
     ...
     module procedure parallel_sum_int_3d
...
subroutine parallel_init_filesystem
  ...
  use_mpiiofs = .true.
  ! specify file system for MPI i/o.
  ! For newer versions of OpenMPI (3.0 or later, accessed on Tiger using
  ! "module load openmpi/intel-17.0" or later, the filesystem type is detected
  ! automatically and cannot be specified by the user. In that case, use_mpiiofs 
  ! should be set to false. In the input file, set the option "MPIIO fs : auto"
  ! to achieve this.
  ...
  case('dws')
     use_mpiiofs = .false.
     mpi_info = MPI_INFO_NULL !! option needed for Cori BB
     !call MPI_INFO_CREATE(mpi_info,ierr)
     !call MPI_INFO_SET(mpi_info,"romio_cb_write","disable",ierr)
  case('auto') 
     mpi_info = MPI_INFO_NULL
     use_mpiiofs = .false.
  case default
