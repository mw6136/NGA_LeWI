module data
  ...
  logical ::       vmol_present
  logical ::       dmol_present

  ! Chemistry model
  character(len=str_medium) :: chemistry
  ! Finitechem variables
  ! First mass fraction scalar index
  integer :: isc_sc
  ! Temperature
  integer :: isc_T
  ! Number of non-steady species
  integer :: N_nons

  ! Switches for approximately factorized Jacobian
  ! -- AF_jac:      
  !       TRUE:  Use monolithic scalar scheme with approximately factorized Jacobian
  !       FALSE: Use operator splitting schemes for scalars (with CVODE)
  ! -- AF_diag_jac:
  !       TRUE:  Use diagonal approximation to the chemical Jacobian
  !       FALSE: Use exact Jacobian (recommended)
  logical :: AF_jac
  logical :: AF_diag_jac

  ! Source terms and heat release rate at every grid point (allocated in finitechem)
  real(WP), dimension(:,:,:,:), pointer :: rhs_fc
  real(WP), dimension(:,:,:), pointer :: HR_gp
  logical :: use_HR=.false.

  ! Arrays to save diffusive fluxes from finitechem_diffusion for output
  ! (Allocated in postprocess/dump_plane.f90)
  real(WP), dimension(:,:,:,:,:), pointer :: diff_flux
  real(WP), dimension(:,:,:,:), pointer ::   diff_flux_div
  logical :: diff_output = .false.

  ! Switch for MMS in scalar_quick_residual
  logical :: mms
...
contains
  ...
  subroutine data_mpi_init
    ...
    do var=1,MPI_IO_NVARS
       ...
       case ('VISCmol')
          MPI_IO_DATA(var)%var => VISCmol(imin_:imax_,jmin_:jmax_,kmin_:kmax_)
       case ('DIFFmol')
          MPI_IO_DATA(var)%var => DIFFmol(imin_:imax_,jmin_:jmax_,kmin_:kmax_,1)
          ! Scalars
       case default
