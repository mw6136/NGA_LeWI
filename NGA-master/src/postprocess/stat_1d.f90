module stat_1d
  ...
  character(len=str_short) :: condtext
...
!!$    ! Raise error if Zstar conditioned stats are requested but Zstar does not exist
!!$    if (cond_zstar .and. (isc_ZSTAR.eq.0)) & 
!!$       call die("stat_1d_init: cannot initialize Z* conditional stats, ZSTAR not defined")
    ...
    if (cond_stat) then
       ...
       ! Chi
       if (use_chi_z) then
          stat_nvar_cond = stat_nvar_cond+2
       end if
       if (use_chi_c) then
          stat_nvar_cond = stat_nvar_cond+2
       end if
    ...
    call parser_read('Chemtable maximum prog', max_prog, 1.0_WP)
    
    if (cond_stat) then
       stat_nvar_cond = 0
       ! Density
       stat_name_cond(stat_nvar_cond+1) = 'RHO' // trim(condtext)
       stat_nvar_cond = stat_nvar_cond + 1
       ! Scalars
       do isc=1,nscalar
          name = SC_name(isc)
          if (name(1:2).ne.'S_') then
             stat_name_cond(stat_nvar_cond+1) = 'SC-'     // trim(adjustl(sc_name(isc))) // trim(condtext)
             stat_name_cond(stat_nvar_cond+2) = 'SC^2-'   // trim(adjustl(sc_name(isc))) // trim(condtext)
             stat_name_cond(stat_nvar_cond+3) = 'rhoS-'   // trim(adjustl(sc_name(isc))) // trim(condtext)
             stat_name_cond(stat_nvar_cond+4) = 'rhoS^2-' // trim(adjustl(sc_name(isc))) // trim(condtext)
             stat_nvar_cond = stat_nvar_cond + 4
          end if
       end do
       ! Scalar Dissipation Rate
       ...
       ! Chi
       if (use_chi_z) then
          stat_name_cond(stat_nvar_cond+1) = 'CHI_ZZ'
          stat_name_cond(stat_nvar_cond+2) = 'CHI_HH'
          stat_nvar_cond = stat_nvar_cond + 2
       end if
       if (use_chi_c) then
          stat_name_cond(stat_nvar_cond+1) = 'CHI_CC'
          stat_name_cond(stat_nvar_cond+2) = 'CHI_HH'
          stat_nvar_cond = stat_nvar_cond + 2
       end if
       ! Combustion
       if (trim(chemistry).eq.'chemtable') then
          stat_name_cond(stat_nvar_cond+ 1) = 'T' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 2) = 'rhoT' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 3) = 'T^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 4) = 'rhoT^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 5) = 'Y_F' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 6) = 'rhoY_F' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 7) = 'Y_F^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 8) = 'rhoY_F^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 9) = 'Y_O2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+10) = 'rhoY_O2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+11) = 'Y_O2^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+12) = 'rhoY_O2^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+13) = 'Y_CO' // trim(condtext)
          stat_name_cond(stat_nvar_cond+14) = 'rhoY_CO' // trim(condtext)
          stat_name_cond(stat_nvar_cond+15) = 'Y_CO^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+16) = 'rhoY_CO^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+17) = 'Y_CO2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+18) = 'rhoY_CO2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+19) = 'Y_CO2^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+20) = 'rhoY_CO2^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+21) = 'Y_H2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+22) = 'rhoY_H2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+23) = 'Y_H2^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+24) = 'rhoY_H2^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+25) = 'Y_H2O' // trim(condtext)
          stat_name_cond(stat_nvar_cond+26) = 'rhoY_H2O' // trim(condtext)
          stat_name_cond(stat_nvar_cond+27) = 'Y_H2O^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+28) = 'rhoY_H2O^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+29) = 'Y_OH' // trim(condtext)
          stat_name_cond(stat_nvar_cond+30) = 'rhoY_OH' // trim(condtext)
          stat_name_cond(stat_nvar_cond+31) = 'Y_OH^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+32) = 'rhoY_OH^2' // trim(condtext)
          stat_nvar_cond = stat_nvar_cond + 32
       end if
       
       ! Source Terms
       if (use_chi_z.or.use_chi_c) then
          stat_name_cond(stat_nvar_cond+1) = 'SRC_H'
          stat_name_cond(stat_nvar_cond+2) = 'SRC_C'
          stat_nvar_cond = stat_nvar_cond + 2
       end if

       ! Soot
       if (use_soot) then
          stat_name_cond(stat_nvar_cond+ 1) = 'fV' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 2) = 'fV^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 3) = 'N' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 4) = 'N^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 5) = 'dp' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 6) = 'dp^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 7) = 'np' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 8) = 'np^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+ 9) = 'intermit' // trim(condtext)
          stat_name_cond(stat_nvar_cond+10) = 'intermit^2' // trim(condtext)
          stat_name_cond(stat_nvar_cond+11) = 'dNdt_nucl' // trim(condtext)
          stat_name_cond(stat_nvar_cond+12) = 'dNdt_coag' // trim(condtext)
          stat_name_cond(stat_nvar_cond+13) = 'dNdt_ox' // trim(condtext)
          stat_name_cond(stat_nvar_cond+14) = 'dNdt_frag' // trim(condtext)
          stat_name_cond(stat_nvar_cond+15) = 'dfvdt_nucl' // trim(condtext)
          stat_name_cond(stat_nvar_cond+16) = 'dfvdt_cond' // trim(condtext)
          stat_name_cond(stat_nvar_cond+17) = 'dfvdt_sg' // trim(condtext)
          stat_name_cond(stat_nvar_cond+18) = 'dfvdt_ox' // trim(condtext)
          stat_nvar_cond = stat_nvar_cond + 18
          if (.not.use_pah) then
             stat_name_cond(stat_nvar_cond+1) = 'Y_PAH' // trim(condtext)
             stat_name_cond(stat_nvar_cond+2) = 'rhoY_PAH' // trim(condtext)
             stat_name_cond(stat_nvar_cond+3) = 'Y_PAH^2' // trim(condtext)
             stat_name_cond(stat_nvar_cond+4) = 'rhoY_PAH^2' // trim(condtext)
             stat_nvar_cond = stat_nvar_cond + 4
          end if
       end if
    end if
...
subroutine stat_1dy_sample
  ...
  ! Calculate necessary scalar dissipation rates
  if (use_chi_c) then
  ...
  do loc=1,nxloc
     ...
     do k=kmin_,kmax_
        do j=jmin_,jmax_
           ...
           ! Chi
           if (use_chi_z) then
              stat_y(loc,j,ns+1) = stat_y(loc,j,ns+1) + dt*sum(cxmloc(loc,:)*CHI(im:im+1,j,k))
              stat_y(loc,j,ns+2) = stat_y(loc,j,ns+2) + dt*sum(cxmloc(loc,:)*tmp11(im:im+1,j,k))
              ns = ns + 2
           end if
           if (use_chi_c) then
              stat_y(loc,j,ns+1) = stat_y(loc,j,ns+1) + dt*sum(cxmloc(loc,:)*tmp10(im:im+1,j,k))
              stat_y(loc,j,ns+2) = stat_y(loc,j,ns+2) + dt*sum(cxmloc(loc,:)*tmp11(im:im+1,j,k))
              ns = ns + 2
           end if
           ...
           ! Src terms
           if (use_chi_z .or. use_chi_c) then
              stat_y(loc,j,ns+1) = stat_y(loc,j,ns+1) + dt*sum(cxmloc(loc,:)*tmp12(im:im+1,j,k))
              stat_y(loc,j,ns+2) = stat_y(loc,j,ns+2) + dt*sum(cxmloc(loc,:)*tmp13(im:im+1,j,k))
              ns = ns + 2
           end if
           ...
           if (cond_stat) then
              ...
              select case (trim(cond_on))
              case ('Z*')
                 bin = min(nbins_cond,max(1,floor(sum(cxmloc(loc,:)*SC(im:im+1,j,k,isc_ZSTAR))*nbins_cond)+1))   
              case ('C')
                 bin = min(nbins_cond,max(1,floor(sum(cxmloc(loc,:)*SC(im:im+1,j,k,isc_PROG)/max_prog)*nbins_cond)+1))
              case default
              ...
              if (use_chi_z) then
                 stat_y_cond(loc,bin,ns+1) = stat_y_cond(loc,bin,ns+1) + sum(cxmloc(loc,:)*CHI(im:im+1,j,k))
                 stat_y_cond(loc,bin,ns+2) = stat_y_cond(loc,bin,ns+2) + sum(cxmloc(loc,:)*tmp11(im:im+1,j,k))
                 ns = ns + 2
              end if
              if (use_chi_c) then
                 stat_y_cond(loc,bin,ns+1) = stat_y_cond(loc,bin,ns+1) + sum(cxmloc(loc,:)*tmp10(im:im+1,j,k))
                 stat_y_cond(loc,bin,ns+2) = stat_y_cond(loc,bin,ns+2) + sum(cxmloc(loc,:)*tmp11(im:im+1,j,k))
                 ns = ns + 2
              end if
              ! Combustion
              ...
              ! Src terms
              if (use_chi_z .or. use_chi_c) then
                 stat_y_cond(loc,bin,ns+1) = stat_y_cond(loc,bin,ns+1) + sum(cxmloc(loc,:)*tmp12(im:im+1,j,k))
                 stat_y_cond(loc,bin,ns+2) = stat_y_cond(loc,bin,ns+2) + sum(cxmloc(loc,:)*tmp13(im:im+1,j,k))
                 ns = ns + 2
              end if
  ...
  do loc=1,nyloc
     ...
     do k=kmin_,kmax_
        do i=imin_,imax_
           ...
           ! Chi
           if (use_chi_z) then
              stat_x(loc,i,ns+1) = stat_x(loc,i,ns+1) + dt*sum(cymloc(loc,:)*CHI(i,jm:jm+1,k))
              stat_x(loc,i,ns+2) = stat_x(loc,i,ns+2) + dt*sum(cymloc(loc,:)*tmp11(i,jm:jm+1,k))
              ns = ns + 2
           end if
           if (use_chi_c) then
              stat_x(loc,i,ns+1) = stat_x(loc,i,ns+1) + dt*sum(cymloc(loc,:)*tmp10(i,jm:jm+1,k))
              stat_x(loc,i,ns+2) = stat_x(loc,i,ns+2) + dt*sum(cymloc(loc,:)*tmp11(i,jm:jm+1,k))
              ns = ns + 2
           end if
           ...
           ! Chi
           if (use_chi_z .or. use_chi_c) then
              stat_x(loc,i,ns+1) = stat_x(loc,i,ns+1) + dt*sum(cymloc(loc,:)*tmp12(i,jm:jm+1,k))
              stat_x(loc,i,ns+2) = stat_x(loc,i,ns+2) + dt*sum(cymloc(loc,:)*tmp13(i,jm:jm+1,k))
              ns = ns + 2
           end if
...
subroutine stat_1dy_cond_write
  ...
  if (allx) then
     ...
     select case (trim(cond_on))
     case ('Z*')
        write(iunit,'(10000a20)') 'ZSTAR', (trim(adjustl(stat_name_cond(var))),var=1,stat_nvar_cond)
     case ('C')
        write(iunit,'(10000a20)') 'PROG', (trim(adjustl(stat_name_cond(var))),var=1,stat_nvar_cond)
     case default
        write(iunit,'(10000a20)') 'ZMIX', (trim(adjustl(stat_name_cond(var))),var=1,stat_nvar_cond)
     end select
     do bin =1,nbins_cond
        write(iunit,'(10000ES20.12)') (real(bin,WP)-0.5_WP)*max_prog/real(nbins_cond,WP),sum(buf_y_cond(:,bin,:),dim=1)/real(nx,WP)
     end do
     close(iclose(iunit))
  else
     do loc=1,nxloc
        ...
        do bin=1,nbins_cond
           write(iunit,'(10000ES20.12)') (real(bin,WP)-0.5_WP)*max_prog/real(nbins_cond,WP),buf_y_cond(loc,bin,:)
        end do
        close(iclose(iunit))
     end do
  end if
