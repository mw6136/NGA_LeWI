...
subroutine dump_init
  ...
  do i=1,nout
     ...
     case ('plane')
        call dump_plane_init
     case ('volume')
        call dump_volume_init
        isave_vol = i
     case ('scalarPDF')
        call dump_scalar_pdf_init
     case default
     end select
  end do
  ...
  do i=1,nout
     ...
     case ('scalarPDF')
        if (int(time/output_freq(i)).NE.output_save(i)) then
           ...
           call monitor_log("SCALAR PDF FILE WRITTEN")
        end if
     ...
     case ('plane')
        if (int(time/output_freq(i)).NE.output_save(i)) then
           output_save(i) = int(time/output_freq(i))
           call dump_plane_3D
           call monitor_log("3D PLANE DATA WRITTEN")
        end if
     ...
     case ('volume')
        if (int(time/output_freq(i)).NE.output_save(i)) then
           output_save(i) = int(time/output_freq(i))
           call dump_volume_3D
           call monitor_log("3D VOLUME DATA WRITTEN")
        end if
...
! ================================== !
! Finalize data dumping procedures   !
! ================================== !
subroutine dump_finalize
  ...
  do i=1,nout
     select case (trim(output_type(i)))
     case ('plane')
        call dump_plane_finalize

     case ('volume')
        call dump_volume_finalize
