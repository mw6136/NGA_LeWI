...
subroutine stat_init
  ...
  if (cond_stat) then
     call parser_read('Conditional statistics variable', cond_on, 'Z')
     call parser_is_defined('Conditioned on Z*', isdef)
     if (isdef) then
        call die("'Conditioned on Z*' input file option is deprecated, use 'Conditional statistics variable'")
     end if
  end if
