...
contains
  ...
  subroutine inflow_file_read
    ...
    character(len=str_medium) :: timestring
    real(WP) :: tmp
    ...
    ! Print to monitor/log when inflow file recycles
    write(timestring,*) time
    if (itime_new.lt.itime) call monitor_log("INFLOW FILE RECYCLE AT TIME" // trim (timestring))
  ...
  subroutine inflow_file_get_interp(yin,zin,yloc,zloc,j,k,wj,wk)
    ...
    yp = mod(yp-grid_y(1),inflow_Ly)+grid_y(1)
    if (yp.lt.grid_y(1)) yp = yp+inflow_Ly
    
    ! Find the point in y
    if (yp.lt.mesh_y(1)) then
       j  = inflow_ny
       yi = mesh_y(inflow_ny)-inflow_Ly
       wj = (mesh_y(1)-yp)/(mesh_y(1)-yi)
    else if (yp.ge.mesh_y(inflow_ny)) then
       j  = inflow_ny
       yi = mesh_y(1)+inflow_Ly
       wj = (yi-yp)/(yi-mesh_y(inflow_ny))
    else
...
subroutine inflow_file_init
  ...
  ! Nothing to be done if no 'file' inflow type
  if (file_inflow.ne.1) then !if (file_inflow.ne.1) return
     phase = 0.0_WP
     walk_step = 0.0_WP
  else
     ...
     inflow_Ly = grid_y(inflow_ny+1)-grid_y(1)
...
subroutine inflow_file_velocity
  ...
  do nflow=1,ninlet
     if (trim(inlet_type(nflow)).eq.'file') then
        !$OMP PARALLEL PRIVATE(theta,jj,jj2,k1,k2,wj,wk,prev,next)
        do j=max(jmino_,inlet(nflow)%jmino),min(jmaxo_,inlet(nflow)%jmaxo)
           !$OMP DO
           do k=kmino_,kmaxo_
              ...
              do var=1,inflow_nvar
                 ...
                 case('U')
                    ...
                    jj = index_cc_y(j,k); jj2=mod(jj,inflow_ny)+1
                    k1 = index_cc_z(j,k); k2=mod(k1,inflow_nz)+1
                    wj = interp_cc_y(j,k)
                    wk = interp_cc_z(j,k)
                    prev =       wk *(wj*inflow_prev(jj,k1,var) + (1.0_WP-wj)*inflow_prev(jj2,k1,var)) + &
                         (1.0_WP-wk)*(wj*inflow_prev(jj,k2,var) + (1.0_WP-wj)*inflow_prev(jj2,k2,var))
                    next =       wk *(wj*inflow_next(jj,k1,var) + (1.0_WP-wj)*inflow_next(jj2,k1,var)) + &
                         (1.0_WP-wk)*(wj*inflow_next(jj,k2,var) + (1.0_WP-wj)*inflow_next(jj2,k2,var))
                 ...
                 case('V')
                    ...
                    jj = index_fy_y(j,k); jj2=mod(jj,inflow_ny)+1
                    ...
                    next =       wk *(wj*inflow_next(jj,k1,var) + (1.0_WP-wj)*inflow_next(jj2,k1,var)) + &
                         (1.0_WP-wk)*(wj*inflow_next(jj,k2,var) + (1.0_WP-wj)*inflow_next(jj2,k2,var))
                    if (((inflow_icyl.eq.0) .and. (icyl.eq.1))) then
                       theta = zm(k)
                 ...
                 case('W')
                    ...
                    jj = index_fz_y(j,k); jj2=mod(jj,inflow_ny)+1
...
subroutine inflow_file_scalar
  ...
  do nflow=1,ninlet
     if (trim(inlet_type(nflow)).eq.'file') then
        !$OMP PARALLEL PRIVATE(isc,jj,jj2,k1,k2,wj,wk,prev,next)
        do j=max(jmino_,inlet(nflow)%jmino),min(jmaxo_,inlet(nflow)%jmaxo)
           !$OMP DO
           do k=kmino_,kmaxo_
