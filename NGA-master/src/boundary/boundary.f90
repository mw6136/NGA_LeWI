...
subroutine boundary_dirichlet(vec,dir)
  ...
  case default
     call die("boundary_dirichlet: invalid direction specified")
  end select
...
subroutine boundary_neumann(vec,dir)
  ...
  case default
     call die("boundary_neumann: invalid direction specified")
  end select
...
subroutine boundary_update_border(A,sym,axis)
  ...
  ! Set Centerline symmetry, check for invalid options
  ...
  case('-')
     coeff = -1.0_WP
  case default
     call die ("boundary_update_border: must specify '+' or '-' symmetry")
  end select

  ! Update ghost points near the centerline, check for invalid options
  select case (trim(axis))
  case ('y')
     call centerline_update_y (A,coeff)
  case('ym')
     call centerline_update_ym(A,coeff)
  case default
     call die("boundary_update_border: must specifiy 'y' or 'ym' as axis")
  end select
